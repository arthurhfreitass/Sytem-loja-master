...
function finalizeOrder(paymentMethod) {
    ...
    if (paymentMethod === 'pix') {
        fetch('http://https://0a3afd9bae3a.ngrok-free.app/create_pix_payment', { // troque SEU_BACKEND pela URL do Flask/ngrok/render
            method: 'POST',
...
function checkPixStatus(paymentId, orderId) {
    const interval = setInterval(() => {
        fetch(`http://https://0a3afd9bae3a.ngrok-free.app/payment_status/${paymentId}`)
            .then(res => res.json())
            .then(data => {
